...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-20 top-0 left-0 transition-all duration-300 ${isScrolled || isMenuOpen ? 'bg-white/70  backdrop-blur-md' : 'bg-transparent '}`}>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 bg-white/70 space-y-1 sm:px-3 ">
              <button
                onClick={() => setIsMenuOpen(false)}
                className="absolute  top-4 right-4 text-gray-900 hover:text-gray-500 focus:outline-none"
              >
...
const MobileNavLink = ({ href, label, onClick }: { href: string; label: string; onClick: () => void }) => (
  <Link href={href} onClick={onClick} className="block border-b overflow-y-auto border-gray-200 px-3 py-3 rounded-md text-lg font-medium text-gray-900 hover:bg-gray-100/20 hover:text-gray-500 transition-colors duration-300">
    {label}
  </Link>
);
